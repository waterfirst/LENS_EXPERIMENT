...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(8,
      ...
      '),
           plotOutput("airyPlot", height = "300px"),
           plotOutput("intensityPlot", height = "300px")
    )
  )
)
...
server <- function(input, output) {
  ...
  generatePlots <- reactive({
    ...
    intensity_1d <- safe_airy_disk(rho_1d)
  ...
  output$intensityPlot <- renderPlot({
    ...
    text(data$zero_peak_radius*1e6, 0.1, 
  ...
  output$downloadPNG <- downloadHandler(
    filename = function() {
      paste("airy_disk_plot_", input$wavelength, "nm_exposure_", input$exposure, "_", Sys.Date(), ".png", sep = "")
    },
  ...
  output$downloadCSV <- downloadHandler(
    filename = function() {
      paste("airy_disk_data_", input$wavelength, "nm_exposure_", input$exposure, "_", Sys.Date(), ".csv", sep = "")
    },
    content = function(file) {
      data <- generatePlots()
      df <- data.frame(
